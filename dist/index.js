...
class SwaggerToTanStackGenerator {
    ...
    baseUrl;
    outputDir;
    constructor(spec, baseUrl, outputDir) {
        this.spec = spec;
    ...
    typeScriptType(property, schemas) {
        if (property.$ref) {
...
`;
    ...
    generateApiFunction(path, method, endpoint) {
...
export const ${this.toCamelCase(functionName)} = async (${params}) => {
  ...
  return data;
    ...
    generateQueryHook(path, method, endpoint) {
        const functionName = endpoint.operationId ||
            `${method}${path.split('/').map(p => p.startsWith('{') ? '' : this.capitalize(p)).join('')}`;
...
export const ${hookName} = (${params}) => {
  return useQuery({
    ...
    generateMutationHook(path, method, endpoint) {
        const functionName = endpoint.operationId ||
            `${method}${path.split('/').map(p => p.startsWith('{') ? '' : this.capitalize(p)).join('')}`;
        const hookName = `use${this.toPascalCase(functionName)}`;
        const apiFunctionName = this.toCamelCase(functionName);
...
export const ${hookName} = () => {
  ...
  return useMutation({
    ...
    generateFiles() {
        ...
        // Write types file
        fs.writeFileSync(path.join(this.outputDir, 'types.ts'), types);
        // Write API file
        fs.writeFileSync(path.join(this.outputDir, 'api.ts'), `${apiImports}
import type {
...
`);
        // Write hooks file
        fs.writeFileSync(path.join(this.outputDir, 'hooks.ts'), `${hookImports}
import {
...
async function loadSwaggerSpec(input) {
    let content;
